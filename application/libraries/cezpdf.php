...
// ------------------------------------------------------------------------------
	...
	//filtering XLS&PDF
	function getFilter($result,$container){
		$n_count=0;$results=array();
		foreach($result as $record){
			$row_result=array();
			foreach($container as $k=>$array){
				foreach($record as $key=>$value){
					if($array['viewreport']==1 && $k==$key){
						$row_result[$array['fields']]=str_replace("<br>","\n",$value);
					}
				}
			}
			$results[]=$row_result;
	...
	function setExcellArray($result,$container){
		$number=array('id_advokat','id_advokat_lama',
					  'id_advokat_pendamping','telepon1',
					  'id_advokat_lama_pendamping','telepon2',
					  'fax1','fax2','fax3',
					  'hp1','hp2','hp','kode_pos',
					  'id_akun','id_tipe_akun',
					  'telepon3','nia_pemimpin',
					  'telepon_rumah1','telepon_rumah2',
					  'fax_rumah','telepon','fax');
		$n_count=0;$results=array();
		foreach($result as $record){
			$row_result=array();
			foreach($container as $k=>$array){
				foreach($record as $key=>$value){
					if( ($array['viewreport']==1 || $array['viewedit']==1 || $array['view']==1) && $k==$key){
	...
	function ezFooter($text, $orientations='portrait', $paper='A4'){
		...
		if($paper=='A4' || $paper==''){
			if($orientation=='portrait' || $orientation=='') {
				...
				$this->ezStartPageNumbers(560,28,8,'','{PAGENUM}',1);
				$this->line(18,40,568,40);
				$this->addText(50,32,8,'Printed on ' . date('m/d/Y h:i:s a'));
				$this->addText(50,22,8,$text);
				//Header
//				$this->ezText('Peradi Document test long text header - '. date('Y'),8,array('justification'=>'right'));
//				$this->addTextWrap(50,605,100,8,"Test Header Long Text Test Header Long Text Test Header Long Text Test Header Long Text Test Header Long Text",'left',0);
				$this->line(18,800,568,800);
				$this->addText(50,805,8,'Peradi Document - '. date('Y'),0,0);
			}else {
				$this->ezSetMargins(50,70,50,50);
				$this->ezStartPageNumbers(760,20,8,'','{PAGENUM}',1);
				$this->line(20,40,810,40);
				$this->addText(50,32,8,'Printed on ' . date('m/d/Y h:i:s a'));
				$this->addText(50,22,8,$text);
				//Header
				$this->line(20,550,810,550);
				$this->addText(50,555,8,'Peradi Document - '. date('Y'),0,0);
			}
		}elseif($paper=='LETTER'){
			if($orientation=='portrait' || $orientation=='') {
				$this->ezSetMargins(30,50,30,30);
				$this->ezStartPageNumbers(550,28,8,'','{PAGENUM}',1);
				$this->line(20,40,580,40);
				$this->addText(50,32,8,'Printed on ' . date('m/d/Y h:i:s a'));
				$this->addText(50,22,8,$text);
				//Header
				$this->line(20,765,580,765);
				$this->addText(50,770,8,'Peradi Document - '. date('Y'),0,0);
			}else {
				$this->ezSetMargins(30,50,20,20);
				$this->ezStartPageNumbers(740,20,8,'','{PAGENUM}',1);
				$this->line(20,40,750,40);
				$this->addText(50,32,8,'Printed on ' . date('m/d/Y h:i:s a'));
				$this->addText(50,22,8,$text);
				//Header
				$this->line(20,585,750,585);
				$this->addText(50,590,8,'Peradi Document - '. date('Y'));
			}
		}else {
			if($orientation=='portrait' || $orientation=='') {
				$this->ezSetMargins(30,50,30,30);
	...
	function umaxwidth($orientation='portrait',$paper='A4'){
		if($paper=='A4' || $paper==''){
			if($orientation=='portrait' || $orientation=='') {
				return 550;
			}else {
