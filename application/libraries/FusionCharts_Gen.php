...
#############============ For Future Use (end)============================
	...
	function FusionCharts_Gen(){
	 ...
	 function FusionCharts($chartType,$width,$height,$chartID=""){
		/*Custom Adding for Reset*/
		/*by Firman*/
		$this->chartParams = array();   #List of Chart Parameters
		$this->chartParamsCounter=0;		#Number of Chart parameters
		
		$this->categoriesParam='';           # Categories Parameter Setting 
		$this->categoryNames = array();     # Category array for storing Category set
		$this->categoryNamesCounter=0;        # Category array counter
		
		$this->dataset = array();         # dataset array
		$this->datasetParam = array();    # dataset parameter setting array
		$this->datasetCounter=0;            # dataset array counter
		$this->setCounter=0;
...
##------------ PUBLIC FUNCTIONS ----------------------------------------------------------------
	 ...
	 // --- BEGIN Chart UJIAN PROFESI ---
	 
	 
	/**/# Setting path of SWF file. file name like FCF_Column3D.swf. where FCF_ is common for all SWF file
	 function setSWFPathUjian($SWFPath){
	   $this->SWFPath=$SWFPath;
	   $this->SWFFile=$this->SWFPath . "FCF_" . $this->chartSWF[$this->chartType][0]  . ".swf";
	 }
	 ...
	 # We can add or change single Chart parameter by setChartParam function
	 # its take Parameter Name and its Value
	 function setChartParamUjian($paramName, $paramValue){
	   $this->chartParams[$paramName]=$this->encodeSpecialChars($paramValue);
	 }
	 ...
	 # its take parameterset [ caption=xyz caption;subCaption=abcd abcd abcd;xAxisName=x axis;yAxisName=y's axis;bgColor=f2fec0;animation=1 ]
	 # Defult Parameter Separator is ";"
	  function setChartParamsUjian($strParam){
	   $listArray=explode($this->del,$strParam);
	   foreach ($listArray as $valueArray) {
	       $paramValue=explode("=",$valueArray);
		   if($this->validateParam($paramValue)==true){
	 ...
	 # Function addCategory adding Category and vLine element
	 function addCategoryUjian($label="",$catParam="",$vlineParam = "" ){
	 	 $strCatXML="";
		 $strParam="";
		 # cheking vlineParam equal blank
		 if($vlineParam==""){
	 ...
	 # adding dataset array element
	 function addDatasetUjian($seriesName, $strParam=""){
	   $this->datasetCounter++;
       $this->createDataset($this->datasetCounter);
	 ...
	 # Function addChartData adding set data element
	 function addChartDataUjian($value="",$setParam="",$vlineParam = "" ){
	     $strSetXML="";
	 ...
	 function renderChartUjian(){
		
		$this->strXML=$this->getXML();	
		$this->SWFFile=$this->SWFPath . "FCF_" . $this->chartSWF[$this->chartType][0]  . ".swf";
	    # print the charts
	    return renderChart($this->SWFFile . $this->chartMSG,"",$this->strXML,$this->chartID, $this->width, $this->height,$this->debugMode,$this->registerWithJS,$this->bgColor,$this->scaleMode,$this->lang);
		
	  } 
	 // --- END Chart UJIAN PROFESI ---
