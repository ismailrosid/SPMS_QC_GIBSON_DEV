...
class Reportlist extends Controller {
	...
	function Reportlist(){
		...
		$this->load->model('ag/Report_model');
	...
	function index($sMessage=''){
		...
		$aDisplay=array('baseurl'			=> base_url(),
						...
						'siteurl'		=> site_url().'/ag/reportlist/',
						'MESSAGES'		=> '',
	...
	function stock($sMessage=''){
		...
		$nFirstStock=$this->Report_model->getLastStock( $sCriteria, $d_production_date_month_filter, $d_production_date_year_filter );
		...
		$aTotalData['n_t_first_stock']=number_format($nFirstStock);
		$aTotalData['n_t_on_progress']=number_format($nOnProgress);
		$aTotalData['n_t_in']=number_format($nIn);
		$aTotalData['n_t_out']=number_format($nOut);
		$aTotalData['n_t_last_stock']=number_format($nFirstStock+$nIn-$nOut);
		...
		if (empty($sMessage)) {
			...
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist/stock/',
							
							'MESSAGES'		=> '',
							'filterCaption'		=> 'AG Stock Filter/Search',
			...
			$aDisplay['PAGE_TITLE']	= 'SPMS-G. Accoustic Guitar/Last Month';
			...
			$aDisplay['viewFilter'] = $this->load->view('ag/report/stock_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/stock', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
	...
	function daily($bExcel=''){
		...
		//$s_location_filter=$this->input->post('s_location_filter');
		//if ( !$this->input->post('s_location_filter') ) $s_location_filter=$s_location_filter);

		$aDataProduct=$this->Report_model->getListDaily($d_production_date_month_filter, $d_production_date_year_filter);


				
		$aDataProductTotal=array('s_code'=>' ','s_name'=>' ');
		...
		$aDataProductTotal['n_t_total']=$n_t_total;
		$aDataProductTotals[]=$aDataProductTotal;
		
		if (empty($bExcel)) {
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist/daily/',
							
							'MESSAGES'		=> '',
							'PAGE_TITLE'		=> 'SPMS-G. Accoustic Guitar/Daily (Production)',
							'filterCaption'		=> 'AG Daily Filter/Search',
			...
			$aDisplay['d_production_date_month_filter'] = $this->form->selectboxarray($this->config->item('list_month'),
			...
			$aDisplay['viewFilter'] = $this->load->view('ag/report/daily_filter', $aDisplay, TRUE);
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/daily', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			// excel exporter
			$this->load->helper('excel');
...
function group($sViewReport, $sMessage=''){
		...
		$n_t_qty=0; $n_t_process_1=0; $n_t_process_2=0; $n_t_process_3=0;
		$n_t_process_4=0; $n_t_process_5=0; $n_t_process_6=0; $n_t_process_7=0;
		$n_t_process_8=0; $n_t_warehouse=0; $n_t_process_9=0; $n_t_process_10=0; $n_t_process_14=0;
		...
		foreach($aDataProduct as $nRow=>$aEachDataProduct){
			...
			if (trim($sViewReport)=='po') {
				$n_t_remark+=$aEachDataProduct['s_notes2'];
			}
		}
		...
		if (trim($sViewReport)=='po') {
			$aTotalData['n_t_remark']=number_format($n_t_remark , 0, ',', '.');
		}
		$aTotalDatas[]=$aTotalData;
		...
		$aDisplay=array('baseurl'			=> base_url(),
						'basesiteurl'		=> site_url(),
						'siteurl'		=> site_url().'/ag/reportlist/group/'.$sViewReport.'/',
						
						'MESSAGES'		=> '',
						'filterCaption'		=> 'AG Group Filter/Search',
		...
		if (trim($sViewReport)=='po') {
			$aDisplay['PAGE_TITLE']	= 'SPMS-G. Accoustic Guitar/Group By PI Number (Stock)';
		}elseif (trim($sViewReport)=='lot') {
			$aDisplay['PAGE_TITLE']	= 'SPMS-G. Accoustic Guitar/Group By LOT Number (Stock)';
		}elseif (trim($sViewReport)=='buyer') {
			$aDisplay['PAGE_TITLE']	= 'SPMS-G. Accoustic Guitar/Group By Buyer(Stock)';
		}elseif (trim($sViewReport)=='model') {
			$aDisplay['PAGE_TITLE']	= 'SPMS-G. Accoustic Guitar/Group By Model (Stock)';
		}elseif (trim($sViewReport)=='color') {
			$aDisplay['PAGE_TITLE']	= 'SPMS-G. Accoustic Guitar/Group By Color (Stock)';
		}
		...
		$aDisplay['viewFilter'] = $this->load->view('ag/report/'.$sViewReport.'_filter', $aDisplay, TRUE);
		
		$this->parser->parse('header', $aDisplay);
		$this->parser->parse('ag/report/'.$sViewReport, $aDisplay);
		$this->parser->parse('footer', $aDisplay);
	}
	...
	function groupexcel($sViewReport) {
		...
		$aHeader = array(
			...
			's_location' => 'Location',
			'n_qty' => 'Qty',
			'n_process_1' => 'WK Center Input',
			'n_process_2' => 'WK Center Output',
			'n_process_3' => 'Wood Working',
			'n_process_4' => 'Coating-I',
			'n_process_5' => 'Sanding',
	...
	function serialdatephase($sMessage=''){
			...
			if ( $this->input->post('s_phase_filter') ) {
				...
				if ($sPhaseName=='d_process_1')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NULL AND ttp.d_process_3 IS NULL AND ttp.d_process_4 IS NULL AND 
						ttp.d_process_5 IS NULL AND ttp.d_process_6 IS NULL AND ttp.d_process_7 IS NULL AND ttp.d_process_8 IS NULL AND ttp.d_process_9 IS NULL AND 
						ttp.d_process_10 IS NULL AND ttp.d_process_14 IS NULL";
				...
				if ($sPhaseName=='d_process_5')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NULL AND ttp.d_process_7 IS NULL AND ttp.d_process_8 IS NULL AND ttp.d_process_9 IS NULL AND 
						ttp.d_process_10 IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_6')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NULL AND ttp.d_process_8 IS NULL AND ttp.d_process_9 IS NULL AND 
						ttp.d_process_10 IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_7')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NULL AND ttp.d_process_9 IS NULL AND 
						ttp.d_process_10 IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_8')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND ttp.d_process_9 IS NULL AND 
						ttp.d_process_10 IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_9')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND ttp.d_process_9 IS NOT NULL AND 
						ttp.d_process_10 IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_10')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND ttp.d_process_9 IS NOT NULL AND 
						ttp.d_process_10 IS NOT NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_14')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND ttp.d_process_9 IS NOT NULL AND 
						ttp.d_process_10 IS NOT NULL AND ttp.d_process_14 IS NOT NULL";
			}
		...
		$n_t_process_4=0; $n_t_process_5=0; $n_t_process_6=0; $n_t_process_7=0; 
		$n_t_process_8=0; $n_t_process_9=0; 
		$n_t_process_10=0; $n_t_process_14=0;
		...
		foreach($aAllDataProduct as $nRow=>$aEachDataProduct){
			...
			$n_t_process_2+=(!empty($aEachDataProduct['d_process_2']) && empty($aEachDataProduct['d_process_3']) && 
							 ...
							 empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_3+=(!empty($aEachDataProduct['d_process_3']) && empty($aEachDataProduct['d_process_4']) && empty($aEachDataProduct['d_process_5']) && 
							 empty($aEachDataProduct['d_process_6']) && empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_4+=(!empty($aEachDataProduct['d_process_4']) && empty($aEachDataProduct['d_process_5']) && empty($aEachDataProduct['d_process_6']) && 
							 empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_5+=(!empty($aEachDataProduct['d_process_5']) && empty($aEachDataProduct['d_process_6']) && empty($aEachDataProduct['d_process_7']) && 
							 empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_6+=(!empty($aEachDataProduct['d_process_6']) && empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_7+=(!empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_8+=(!empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_9+=(!empty($aEachDataProduct['d_process_9']) && empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
		...
		if (empty($sMessage)) {
			...
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist/serialdatephase/',
							
							'MESSAGES'		=> '',
							'PAGE_TITLE'		=> 'SPMS-G. Accoustic Guitar/Serial No, Date & Phase (Production)',
			...
			$aDisplay['d_production_date_month_filter'] = $this->form->selectboxarray($this->config->item('list_month'), $aSearchForm['d_production_date_month_filter']);
			$aDisplay['d_production_date_month_filter2'] = $this->form->selectboxarray($this->config->item('list_month'), $aSearchForm['d_production_date_month_filter2']);
			$aDisplay['s_phase_filter'] = $this->form->selectbox('tm_prod_setup', 's_field_process, s_phase || \' - \' || s_description AS s_description', "s_division='AG' AND n_line=1 AND s_field_process<>'d_warehouse' ORDER BY n_order ASC, s_phase ASC", 's_field_process', 's_description', $sPhaseName);
			$aDisplay['s_buyer_filter'] = $this->form->selectboxquery("SELECT s_code, s_code || ' - ' || s_name AS s_name FROM tm_customer WHERE s_division='AG' OR s_division IS NULL ORDER BY s_name ASC", 's_code', 's_name', $aDisplay['s_buyer_filter']);
			
			$aDisplay['s_location_filter'] = $this->form->selectboxarray($this->config->item('product_location'), 
			$aSearchForm['s_location_filter']);
			$aDisplay['viewFilter'] = $this->load->view('ag/report/serialdatephase_filter', $aDisplay, TRUE);

			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/serialdatephase', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
	...
	function serialphase($sMessage=''){
		...
		if( isset($_POST['nOffset']) ){
			...
			if ( $this->input->post('s_phase_filter') ) {
				...
				if ($sPhaseName=='d_process_1')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NULL AND ttp.d_process_3 IS NULL AND ttp.d_process_4 IS NULL AND 
						...
						ttp.d_process_9 IS NULL AND ttp.d_process_10 IS NULL AND ttp.d_warehouse IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_2')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NULL AND ttp.d_process_4 IS NULL AND 
						ttp.d_process_5 IS NULL AND ttp.d_process_6 IS NULL AND ttp.d_process_7 IS NULL AND ttp.d_process_8 IS NULL AND 
						ttp.d_process_9 IS NULL AND ttp.d_process_10 IS NULL AND ttp.d_warehouse IS NULL AND ttp.d_process_14 IS NULL";
				...
				if ($sPhaseName=='d_process_9')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND 
						ttp.d_process_9 IS NOT NULL AND ttp.d_process_10 IS NULL AND ttp.d_warehouse IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_10')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND 
						ttp.d_process_9 IS NOT NULL AND ttp.d_process_10 IS NOT NULL AND ttp.d_warehouse IS NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_warehouse')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND 
						ttp.d_process_9 IS NOT NULL AND ttp.d_process_10 IS NOT NULL AND ttp.d_warehouse IS NOT NULL AND ttp.d_process_14 IS NULL";
				if ($sPhaseName=='d_process_14')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL AND ttp.d_process_2 IS NOT NULL AND ttp.d_process_3 IS NOT NULL AND ttp.d_process_4 IS NOT NULL AND 
						ttp.d_process_5 IS NOT NULL AND ttp.d_process_6 IS NOT NULL AND ttp.d_process_7 IS NOT NULL AND ttp.d_process_8 IS NOT NULL AND 
						ttp.d_process_9 IS NOT NULL AND ttp.d_process_10 IS NOT NULL AND ttp.d_warehouse IS NOT NULL AND ttp.d_process_14 IS NOT NULL";
			}
			if ( $this->input->post('d_transaction_date_filter') ) {
				$d_transaction_date_filter=$this->input->post('d_transaction_date_filter');
				$aCriteria[]="TO_CHAR(ttp.".$this->input->post('s_phase_filter').", 'YYYY-MM-DD') = '$d_transaction_date_filter'";
			...
			if (empty($aSessionSearch['s_phase_filter'])) {
				...
				$sPartCriteria="ttp.$s_phase_filter IS NOT NULL AND	ttp.d_process_2 IS NULL AND 
					ttp.d_process_3 IS NULL AND ttp.d_process_4 IS NULL AND 
					ttp.d_process_5 IS NULL AND ttp.d_process_6 IS NULL AND 
					ttp.d_process_7 IS NULL AND ttp.d_process_8 IS NULL AND ttp.d_warehouse IS NULL AND 
		...
		$n_t_process_5=0; $n_t_process_6=0; $n_t_process_7=0; $n_t_process_8=0; 
		$n_t_warehouse=0; $n_t_process_9=0; $n_t_process_10=0; $n_t_process_14=0;
		...
		foreach($aAllDataProduct as $nRow=>$aEachDataProduct){
			$n_t_process_1+=(!empty($aEachDataProduct['d_process_1']) && empty($aEachDataProduct['d_process_2']) && empty($aEachDataProduct['d_process_3']) && 
							 empty($aEachDataProduct['d_process_4']) && empty($aEachDataProduct['d_process_5']) && empty($aEachDataProduct['d_process_6']) && 
							 empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_2+=(!empty($aEachDataProduct['d_process_2']) && empty($aEachDataProduct['d_process_3']) && 
							 empty($aEachDataProduct['d_process_4']) && empty($aEachDataProduct['d_process_5']) && empty($aEachDataProduct['d_process_6']) && 
							 empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_3+=(!empty($aEachDataProduct['d_process_3']) && empty($aEachDataProduct['d_process_4']) && empty($aEachDataProduct['d_process_5']) && 
							 empty($aEachDataProduct['d_process_6']) && empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && 
							 empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_4+=(!empty($aEachDataProduct['d_process_4']) && empty($aEachDataProduct['d_process_5']) && empty($aEachDataProduct['d_process_6']) && 
							 empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_5+=(!empty($aEachDataProduct['d_process_5']) && empty($aEachDataProduct['d_process_6']) && empty($aEachDataProduct['d_process_7']) && 
							 empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_6+=(!empty($aEachDataProduct['d_process_6']) && empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && 
							 empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_7+=(!empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			...
			$n_t_process_9+=(!empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_10+=(!empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
		...
		if (empty($sMessage)) {
			...
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist/serialphase/',
							
							'MESSAGES'		=> '',
							'PAGE_TITLE'		=> 'SPMS-G. Accoustic Guitar/Serial No & Phase (Stock)',
							'filterCaption'		=> 'AG Report Filter/Search',
			...
			$aDisplay['d_production_date_month_filter'] = $this->form->selectboxarray($this->config->item('list_month'), $aSearchForm['d_production_date_month_filter']);
			$aDisplay['d_production_date_month_filter2'] = $this->form->selectboxarray($this->config->item('list_month'), $aSearchForm['d_production_date_month_filter2']);
			$aDisplay['s_phase_filter'] = $this->form->selectbox('tm_prod_setup', 's_field_process, s_phase || \' - \' || s_description AS s_description', "s_division='AG' AND n_line=1 ORDER BY n_order ASC, s_phase ASC", 's_field_process', 's_description', $sPhaseName);
			$aDisplay['s_buyer_filter'] = $this->form->selectboxquery("SELECT s_code, s_code || ' - ' || s_name AS s_name FROM tm_customer WHERE s_division='AG' OR s_division IS NULL ORDER BY s_name ASC", 's_code', 's_name', $aDisplay['s_buyer_filter']);
			$aDisplay['s_location_filter'] = $this->form->selectboxarray($this->config->item('product_location'), 
				$aSearchForm['s_location_filter']);
			$aDisplay['viewFilter'] = $this->load->view('ag/report/serialphase_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/serialphase', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
	...
	function serialdate($sMessage=''){
		...
		foreach($aAllDataProduct as $nRow=>$aEachDataProduct){
			...
			$n_t_process_8+=(!empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_9+=(!empty($aEachDataProduct['d_process_9']) && empty($aEachDataProduct['d_process_10']) && 
			...
			$n_t_process_10+=(!empty($aEachDataProduct['d_process_10']) && 
							 empty($aEachDataProduct['d_warehouse']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_warehouse+=(!empty($aEachDataProduct['d_warehouse']) && 
							 empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_14+=(!empty($aEachDataProduct['d_process_14']) ? 1 : 0);
		}
		$aTotalData['n_t_process_1']=$n_t_process_1;
		$aTotalData['n_t_process_2']=$n_t_process_2;
		...
		if (empty($sMessage)) {
			...
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist/serialdate/',
							
							'MESSAGES'		=> '',
							'PAGE_TITLE'		=> 'SPMS-G. Accoustic Guitar/Serial No & Date (Stock)',
							'filterCaption'		=> 'AG Report Filter/Search',
			...
			$aDisplay['viewFilter'] = $this->load->view('ag/report/serialdate_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/serialdate', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
	...
	function serial($sMessage=''){
		...
		if (empty($sMessage)) {
			...
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'			=> site_url().'/ag/reportlist/serial/',
							
							'MESSAGES'			=> '',
							'PAGE_TITLE'		=> 'SPMS-G. Accoustic Guitar/Serial No (Stock)',
							'filterCaption'		=> 'AG Report Filter/Search',
			...
			$aDisplay['viewFilter'] = $this->load->view('ag/report/serial_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/serial', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
			...
			if (count($aAllDataProduct)>0) {
				...
				foreach($aAllDataProduct[0] as $sField=>$sValue) {
					if (isset($aHeader[$sField])) {
						if (substr($sField,0,9)=='n_process') {
							$aTotalDatas[$sField] = $aTotalData['n_t_process_'.($nCounter >= 11 ? ($nCounter + 3) : $nCounter)];
							$nCounter++;
...
function rekapOrderPackingExp($sMessage = '')
	{

		$cases = array('order', 'packing', 'export');
		$year = array();
		/* -- Searching -- */
		$aSessionSearch = $this->session->userdata('serial_search');
		$aSearchForm = array(
		...
		if (isset($_POST['nOffset'])) {
			...
			if ($this->input->post('d_production_date_year_filter')) {
				$d_production_date_year_filter = $this->input->post('d_production_date_year_filter');
				// $aCriteria[] = "EXTRACT(YEAR FROM ttp.d_production_date)>='$d_production_date_year_filter'";
				$year[] = $d_production_date_year_filter;
			}
			if ($this->input->post('d_production_date_year_filter2')) {
				$d_production_date_year_filter = $this->input->post('d_production_date_year_filter2');
				// $aCriteria[] = "EXTRACT(YEAR FROM ttp.d_production_date)<='$d_production_date_year_filter'";
				$year[] = $d_production_date_year_filter;
			}
			...
			if (empty($aSessionSearch['d_production_date_year_filter'])) {
				...
				$d_production_date_year_filter = date('Y', strtotime('-1 year'));
				$year[] = $d_production_date_year_filter;
				// $sPartCriteria = "EXTRACT(YEAR FROM ttp.d_production_date)>='$d_production_date_year_filter'";
				//$sPartCriteria="EXTRACT(MONTH FROM ttp.d_production_date)='$d_production_date_month_filter' AND EXTRACT(YEAR FROM ttp.d_production_date)='$d_production_date_year_filter'";
				// $aCriteria[] = $sPartCriteria;
			...
			if (empty($aSessionSearch['d_production_date_year_filter2'])) {
				$d_production_date_year_filter = date('Y');
				$year[] = $d_production_date_year_filter;
				// $sPartCriteria = "EXTRACT(YEAR FROM ttp.d_production_date)<='$d_production_date_year_filter'";
				// $aCriteria[] = $sPartCriteria;
		...
		foreach ($allYear as $year) {
			foreach ($cases as $case) {
				$aAllDataProduct[$year]['tahun'] = $year;
				$aAllDataProduct[$year][$case] = $this->Report_model->getRekapOPE($year, $case);
			}
		}
		...
		foreach ($aAllDataProduct as $nRow => $aEachDataProduct) {
			$t_order += $aEachDataProduct['order'];
			$t_packing += $aEachDataProduct['packing'];
			$t_export += $aEachDataProduct['export'];
		}
		foreach ($aAllDataProduct as $key => $row) {
			foreach ($row as $field => $value) {
				if ($field !== 'tahun') {
					$aAllDataProduct[$key][$field] = number_format($value);
				}
			}
		}
		$aTotalData['t_order'] = number_format($t_order);
		$aTotalData['t_packing'] = number_format($t_packing);
		$aTotalData['t_export'] = number_format($t_export);

		$aTotalDatas[] = $aTotalData;

		if (empty($sMessage)) {

			$aDataProduct = $aAllDataProduct;

			$aDisplay = array(
				'baseurl'			=> base_url(),
				'basesiteurl'		=> site_url(),
				'siteurl'			=> site_url() . '/ag/reportlist/rekaporderpackingexp/',

				'MESSAGES'			=> '',
				'PAGE_TITLE'		=> 'SPMS-G. Accoustic Guitar Rekap Order',
				'filterCaption'		=> 'AG Report Filter/Search',
				...
				'nTotalRows'		=> $nTotalRows,
				'nCurrOffset'		=> $nOffset,
				'tt_rekap'	=> $aDataProduct,
				't_rekap_total'	=> $aTotalDatas
			);

			$aDisplay = array_merge($aDisplay, $aSearchForm);

			$aDisplay['viewFilter'] = $this->load->view('ag/report/rekapope_filter', $aDisplay, TRUE);

			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report/rekapope', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
			...
			$aTotalDatas = array();
			if (count($aAllDataProduct) > 0) {
				$aTotalDatas['division'] = 'AG';
				$aTotalDatas['t_order'] = number_format($t_order);
				$aTotalDatas['t_packing'] = number_format($t_packing);
				$aTotalDatas['t_export'] = number_format($t_export);
			}
			$aDatas[] = $aTotalDatas;
			$name = 'Rekap_order_AG_' . $allYear[0] . '-' . end($allYear);
			to_excel_array($aDatas, $name);
		}
	}
