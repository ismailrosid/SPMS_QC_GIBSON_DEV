...
class Reportlist2 extends Controller {
	var $sUsername;
	var $sLevel;
	...
	function index($sMessage=''){
		...
		$aDisplay=array('baseurl'			=> base_url(),
						...
						'siteurl'		=> site_url().'/ag/reportlist2/',
						'MESSAGES'		=> '',
	...
	function group($sViewReport, $sMessage=''){
		...
		$n_t_process_8=0; $n_t_process_9=0; $n_t_process_10=0; $n_t_process_14=0; 
		if (trim($sViewReport)=='po') {
			...
			$aDataProduct=$this->Report2_model->getListModel($sCriteria, 0, 0, $aSort);
		} elseif(trim($sViewReport)=='color') {
			$aDataProduct=$this->Report2_model->getListColor($sCriteria, 0, 0, $aSort);
		}
		...
		$aDisplay=array('baseurl'			=> base_url(),
						'basesiteurl'		=> site_url(),
						'siteurl'		=> site_url().'/ag/reportlist2/group/'.$sViewReport.'/',
		...
		$aDisplay['viewFilter'] = $this->load->view('ag/report2/'.$sViewReport.'_filter', $aDisplay, TRUE);
		
		$this->parser->parse('header', $aDisplay);
		$this->parser->parse('ag/report2/'.$sViewReport, $aDisplay);
		$this->parser->parse('footer', $aDisplay);
	}
	...
	function serialdatephase($sMessage=''){
		...
		if( isset($_POST['nOffset']) ){
			...
			if ( $this->input->post('s_phase_filter') ) {
				...
				if ($sPhaseName=='d_process_1')
					$aCriteria[]="ttp.d_process_1 IS NOT NULL";
				if ($sPhaseName=='d_process_2')
					$aCriteria[]="ttp.d_process_2 IS NOT NULL";
				if ($sPhaseName=='d_process_3')
					$aCriteria[]="ttp.d_process_3 IS NOT NULL";
				if ($sPhaseName=='d_process_4')
					$aCriteria[]="ttp.d_process_4 IS NOT NULL";
				if ($sPhaseName=='d_process_5')
					$aCriteria[]="ttp.d_process_5 IS NOT NULL";
				if ($sPhaseName=='d_process_6')
					$aCriteria[]="ttp.d_process_6 IS NOT NULL";
				if ($sPhaseName=='d_process_7')
					$aCriteria[]="ttp.d_process_7 IS NOT NULL";
				if ($sPhaseName=='d_process_8')
					$aCriteria[]="ttp.d_process_8 IS NOT NULL";
				if ($sPhaseName=='d_process_9')
					$aCriteria[]="ttp.d_process_9 IS NOT NULL";
				if ($sPhaseName=='d_process_10')
					$aCriteria[]="ttp.d_process_10 IS NOT NULL";
				if ($sPhaseName=='d_process_14')
					$aCriteria[]="ttp.d_process_14 IS NOT NULL";
			}
			...
			if (empty($aSessionSearch['s_phase_filter'])) {
				...
				$sPartCriteria="ttp.$s_phase_filter IS NOT NULL";
				$aCriteria[]=$sPartCriteria;
		...
		$n_t_process_4=0; $n_t_process_5=0; $n_t_process_6=0; $n_t_process_7=0;
		$n_t_process_8=0; $n_t_process_9=0; $n_t_process_10=0; $n_t_process_14=0;
		$aAllDataProduct=$this->Report2_model->getListSerialDatePhase($sCriteria, 0, 0, $aSort);
		$nTotalRows=count($aAllDataProduct);
		
		foreach($aAllDataProduct as $nRow=>$aEachDataProduct){
			$n_t_process_1+=($aEachDataProduct['d_process_1'] != '1900-01-01' && !empty($aEachDataProduct['d_process_1']) ? 1 : 0);
			$n_t_process_2+=($aEachDataProduct['d_process_2'] != '1900-01-01' && !empty($aEachDataProduct['d_process_2']) ? 1 : 0);
			$n_t_process_3+=($aEachDataProduct['d_process_3'] != '1900-01-01' && !empty($aEachDataProduct['d_process_3']) ? 1 : 0);
			$n_t_process_4+=($aEachDataProduct['d_process_4'] != '1900-01-01' && !empty($aEachDataProduct['d_process_4']) ? 1 : 0);
			$n_t_process_5+=($aEachDataProduct['d_process_5'] != '1900-01-01' && !empty($aEachDataProduct['d_process_5']) ? 1 : 0);
			$n_t_process_6+=($aEachDataProduct['d_process_6'] != '1900-01-01' && !empty($aEachDataProduct['d_process_6']) ? 1 : 0);
			$n_t_process_7+=($aEachDataProduct['d_process_7'] != '1900-01-01' && !empty($aEachDataProduct['d_process_7']) ? 1 : 0);
			$n_t_process_8+=($aEachDataProduct['d_process_8'] != '1900-01-01' && !empty($aEachDataProduct['d_process_8']) ? 1 : 0);
			$n_t_process_9+=($aEachDataProduct['d_process_9'] != '1900-01-01' && !empty($aEachDataProduct['d_process_9']) ? 1 : 0);
			$n_t_process_10+=($aEachDataProduct['d_process_10'] != '1900-01-01' && !empty($aEachDataProduct['d_process_10']) ? 1 : 0);
			$n_t_process_14+=($aEachDataProduct['d_process_14'] != '1900-01-01' && !empty($aEachDataProduct['d_process_14']) ? 1 : 0);
		}
		$aTotalData['n_t_process_1']=$n_t_process_1;
		$aTotalData['n_t_process_2']=$n_t_process_2;
		$aTotalData['n_t_process_3']=$n_t_process_3;
		...
		if (empty($sMessage)) {
			$aDataProduct=$this->Report2_model->getListSerialDatePhase($sCriteria, $nLimit, $nOffset, $aSort);
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist2/serialdatephase/',
			...
			$aDisplay['viewFilter'] = $this->load->view('ag/report2/serialdatephase_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report2/serialdatephase', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
	...
	function serialdate($sMessage=''){
		...
		$aAllDataProduct=$this->Report2_model->getListSerialDate($sCriteria, 0, 0, $aSort);
		$nTotalRows=count($aAllDataProduct);
		
		foreach($aAllDataProduct as $nRow=>$aEachDataProduct){
			$n_t_process_1+=(!empty($aEachDataProduct['d_process_1']) && $aEachDataProduct['d_process_1'] != '1900-01-01' ? 1 : 0);
			$n_t_process_2+=(!empty($aEachDataProduct['d_process_2']) && $aEachDataProduct['d_process_2'] != '1900-01-01' ? 1 : 0);
			$n_t_process_3+=(!empty($aEachDataProduct['d_process_3']) && $aEachDataProduct['d_process_3'] != '1900-01-01' ? 1 : 0);
			$n_t_process_4+=(!empty($aEachDataProduct['d_process_4']) && $aEachDataProduct['d_process_4'] != '1900-01-01' ? 1 : 0);
			$n_t_process_5+=(!empty($aEachDataProduct['d_process_5']) && $aEachDataProduct['d_process_5'] != '1900-01-01' ? 1 : 0);
			$n_t_process_6+=(!empty($aEachDataProduct['d_process_6']) && $aEachDataProduct['d_process_6'] != '1900-01-01' ? 1 : 0);
			$n_t_process_7+=(!empty($aEachDataProduct['d_process_7']) && $aEachDataProduct['d_process_7'] != '1900-01-01' ? 1 : 0);
			$n_t_process_8+=(!empty($aEachDataProduct['d_process_8']) && $aEachDataProduct['d_process_8'] != '1900-01-01' ? 1 : 0);
			$n_t_process_9+=(!empty($aEachDataProduct['d_process_9']) && $aEachDataProduct['d_process_9'] != '1900-01-01' ? 1 : 0);
			$n_t_process_10+=(!empty($aEachDataProduct['d_process_10']) && $aEachDataProduct['d_process_10'] != '1900-01-01' ? 1 : 0);
			$n_t_process_14+=(!empty($aEachDataProduct['d_process_14']) && $aEachDataProduct['d_process_14'] != '1900-01-01' ? 1 : 0);
		}
		$aTotalData['n_t_process_1']=$n_t_process_1;
		$aTotalData['n_t_process_2']=$n_t_process_2;
		$aTotalData['n_t_process_3']=$n_t_process_3;
		...
		if (empty($sMessage)) {
			$aDataProduct=$this->Report2_model->getListSerialDate($sCriteria, $nLimit, $nOffset, $aSort);
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/ag/reportlist2/serialdate/',
			...
			$aDisplay['viewFilter'] = $this->load->view('ag/report2/serialdate_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('ag/report2/serialdate', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
