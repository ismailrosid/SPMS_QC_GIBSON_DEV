...
class qcGibsonReport extends Controller
{
    ...
    function index($sMessage = '')
    {
        ...
        $aDisplay = array(
            ...
            'siteurl'        => site_url() . '/ag/qcgibson/report/model/',
            'MESSAGES'        => '',
    ...
    function group($sViewReport, $sMessage = '')
    {
        ...
        $aDisplay = array(
            'baseurl'            => base_url(),
            'basesiteurl'        => site_url(),
            'siteurl'        => site_url() . '/ag/qcgibsonreport/group/' . $sViewReport . '/',
        ...
        $aDisplay['s_buyer_filter'] = $this->form->selectboxquery("SELECT s_code, s_code || ' - ' || s_name AS s_name FROM tm_customer WHERE s_division='AG' AND UPPER(s_name::text) LIKE '%GIBSON%' ORDER BY s_name ASC", 's_code', 's_name', $aDisplay['s_buyer_filter']);
        ...
        $aDisplay['viewFilter'] = $this->load->view('ag/qcgibson/report/' . $sViewReport . '_filter', $aDisplay, TRUE);

        $this->parser->parse('header', $aDisplay);
        $this->parser->parse('ag/qcgibson/report/' . $sViewReport, $aDisplay);
        $this->parser->parse('footer', $aDisplay);
    }
    ...
    function groupexcel($sViewReport)
    {
        ...
        $aHeader = array(
            ...
            'n_process_8' => 'Assembly-I Control Center',
            'n_process_9' => 'Assembly-II',
            'n_process_gibson' => 'Gibson Process',
        ...
        foreach ($aDataProduct as $nRow => $aEachDataProduct) {
            ...
            $aEachDataProduct['n_warehouse'] = rand(10, 50);
