...
class Transaction extends Controller {
	...
	function index($sDivision, $sMessage=''){
		...
		if( isset($_POST['nOffset']) ){
			...
			if (empty($aSessionSearch['d_transaction_date_filter'])) {
				...
				$aSearch['d_transaction_date_filter']=$sPartCriteria;
				$aSearchForm['d_transaction_date_filter']=$d_transaction_date_filter;
			}
	...
	function add($sDivision) {
		...
		if ($this->validation->run() == FALSE ){
			...
			$aData=array(); $aSerialError=array();
	...
	function export_list($sDivision, $sMessage='', $is_export=0)
	{
		...
		$aSearchForm=array(
			'd_transaction_date_input'		=> '', 's_product_location_input'		=> '',
			's_type_filter'					=> '', 
			's_serial_no_filter'			=> '', 's_serial_no2_filter'			=> '', 
		...
		if ($is_export == 0) {
			...
			$aDisplay['s_product_location_input'] = $this->form->selectboxarray($this->config->item('product_location'), $aDisplay['s_product_location_input']);
	...
	function export_action($sDivision, $sSerialNo='')
	{
		...
		if ($rSerialNo===FALSE) {
			...
			$aSessionSearchForm['s_product_location_input'] = $s_product_location;
