...
class Order extends Controller
{
	...
	function viewedit($sDivision, $uId = '')
	{
		...
		foreach ($this->Order_model->aContainer as $sKey => $aProperties) {
			if ($sKey == 's_model') {
				...
				$aDataEditable[$sKey] =	$this->form->selectSearch('tm_model', 's_code, s_description AS s_description, s_description AS s_names', "s_status='Active' AND  (s_division='" . strtoupper($sDivision) . "' OR s_division IS NULL) ORDER BY s_names ASC", 's_code', 's_description', $aEditDefault[$sKey]);
				//} elseif ($sKey=='s_smodel') {
				//	$aDataEditable[$sKey] = $this->form->selectbox('tm_model', 's_smodel, s_smodel || \' - \' || s_description AS s_description, s_description AS s_names', "s_division='".strtoupper($sDivision)."' OR s_division IS NULL ORDER BY s_names ASC", 's_code', 's_description', $aEditDefault[$sKey]);
				...
				$this->session->set_userdata('s_color', $aEditDefault['s_model']);
			} elseif ($sKey == 's_buyer') {
				...
				$aDataEditable[$sKey] =	$this->form->selectSearch('tm_customer', 's_code, s_code || \' - \' || s_name AS s_name', "s_status='Active' AND  (s_division='" . strtoupper($sDivision) . "' OR s_division IS NULL)", 's_code', 's_name', $aEditDefault[$sKey]);
		...
		$aDisplay = array(
			'formaction'		=> site_url() . '/production/order/' . (trim($uId) == '' || $uId == '0' ? 'add/' . $sDivision : 'edit/' . $sDivision . '/' . $uId),
	...
	function edit($sDivision, $uId)
	{
		...
		if ($this->validation->run() == FALSE) {
			...
			$uId = $this->Order_model->update('u_id', $uId, $aData, strtoupper($sDivision));
			if ($uId === FALSE) {
				$sMessages = 0;
			} else {
				$sMessages = 1;
	...
	function getColor()
	{
		$results = array();
		$sCode = $_POST['s_code'];
		$defaultData = $this->session->userdata('s_color');
		$this->session->unset_userdata('s_color');
		$data = empty($sCode) ? $this->Order_model->getColor($defaultData) : $this->Order_model->getColor($sCode);
		if (empty($data)) {
			$results = array(
				"s_code" => "null",
				"s_description" => ""
