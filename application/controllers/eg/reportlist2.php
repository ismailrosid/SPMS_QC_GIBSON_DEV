...
class Reportlist2 extends Controller {
	...
	function index($sMessage=''){
		...
		$aDisplay=array('baseurl'			=> base_url(),
						...
						'siteurl'			=> site_url().'/eg/reportlist2/',
						'MESSAGES'			=> '',
	...
	function group($sViewReport, $sMessage=''){
		...
		$n_t_process_1_2=0; 
		$n_t_process_7=0; $n_t_process_8=0; $n_t_process_9=0;
		...
		$aDisplay=array('baseurl'			=> base_url(),
						'basesiteurl'		=> site_url(),
						'siteurl'		=> site_url().'/eg/reportlist2/group/'.$sViewReport.'/',
		...
		$aDisplay['viewFilter'] = $this->load->view('eg/report2/'.$sViewReport.'_filter', $aDisplay, TRUE);
		
		$this->parser->parse('header', $aDisplay);
		$this->parser->parse('eg/report2/'.$sViewReport, $aDisplay);
		$this->parser->parse('footer', $aDisplay);
	}
	...
	function groupexcel($sViewReport) {
		...
		$aHeader = array(
			...
			'n_process_1_2' => 'WK-I Center Input [Bolt-Body]',
	...
	function serialdatephase($sMessage=''){
		...
		if (empty($sMessage)) {
			...
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'			=> site_url().'/eg/reportlist2/serialdatephase/',
			...
			$aDisplay['viewFilter'] = $this->load->view('eg/report2/serialdatephase_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('eg/report2/serialdatephase', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
	...
	function serialdate($sMessage=''){
		...
		if ($n_line_filter=='1') {
			...
			$aAllDataProduct=$this->Report2_model->getListSerialDate2($sCriteria, 0, 0, $aSort);
		}
		$nTotalRows=count($aAllDataProduct);
		
		foreach($aAllDataProduct as $nRow=>$aEachDataProduct){
			$n_t_process_1s+=(!empty($aEachDataProduct['d_process_1s']) && empty($aEachDataProduct['d_process_2']) && empty($aEachDataProduct['d_process_3']) && 
			...
			$n_t_process_1+=(!empty($aEachDataProduct['d_process_1']) && empty($aEachDataProduct['d_process_2']) && empty($aEachDataProduct['d_process_3']) && 
							 ...
							 empty($aEachDataProduct['d_process_7']) && empty($aEachDataProduct['d_process_8']) && empty($aEachDataProduct['d_process_9']) && 
							 empty($aEachDataProduct['d_process_10']) && empty($aEachDataProduct['d_process_14']) ? 1 : 0);
			$n_t_process_1_2+=(!empty($aEachDataProduct['d_process_1_2b']) && 
							 empty($aEachDataProduct['d_process_2']) ? 1 : 0);
		...
		if (empty($sMessage)) {
			if ($n_line_filter=='1') {
				...
				$aDataProduct=$this->Report2_model->getListSerialDate2($sCriteria, $nLimit, $nOffset, $aSort);
			}
			$aDisplay=array('baseurl'			=> base_url(),
							'basesiteurl'		=> site_url(),
							'siteurl'		=> site_url().'/eg/reportlist2/serialdate/',
			...
			$aDisplay['viewFilter'] = $this->load->view('eg/report2/serialdate_filter', $aDisplay, TRUE);
			
			$this->parser->parse('header', $aDisplay);
			$this->parser->parse('eg/report2/serialdate', $aDisplay);
			$this->parser->parse('footer', $aDisplay);
		} else {
			$this->load->helper('excel');
			$aHeader = array(
			...
			$aTotalDatas=array();
			if (count($aAllDataProduct)>0) {
				$nCounter=1; $nCounter2=1;
				foreach($aAllDataProduct[0] as $sField=>$sValue) {
					if (isset($aHeader[$sField])) {
						if (substr($sField,0,9)=='d_process' && substr($sField,-3)!='_2b') {
							$aTotalDatas[$sField] = $aTotalData['n_t_process_'.($nCounter >= 11 ? ($nCounter + 3) : $nCounter)];
