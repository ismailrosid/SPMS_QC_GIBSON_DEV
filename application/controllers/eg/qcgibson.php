...
class QcGibson extends Controller
{
    ...
    const DIVISION = 'EG'; // Electric Guitar Division
    ...
    function QcGibson()
    {
        ...
        if (!$this->session->userdata('b_eg_cheker_gibson')) {
    ...
    {
        $messages = array('1' => 'Data has been successfully saved.', '2' => 'File upload failed.', '3' => 'Data has been deleted.');
        $aDisplay = array(
            'baseurl'          => base_url(),
            'basesiteurl'      => site_url(),
            'siteurl'          => site_url() . '/eg/qcgibson/',
            'PAGE_TITLE'       => 'SPMS-G. QC Gibson Report (EG)',
        ...
        $this->parser->parse('eg/qcgibson/index', $aDisplay);
    ...
    function scan()
    {
        ...
        $this->parser->parse('header', $aDisplay);
        $this->parser->parse('eg/qcgibson/scan', $aDisplay);
        $this->parser->parse('footer', $aDisplay);
    }
    ...
    function direct()
    {
        ...
        $this->parser->parse('header', $aDisplay);
        $this->parser->parse('eg/qcgibson/direct', $aDisplay);
        $this->parser->parse('footer', $aDisplay);
    }
