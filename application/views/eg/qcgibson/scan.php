    ...
    <div class="card-form-section-body">
      ...
      <div class="error-card" id="errorCard">
        <div class="error-header">
          <div class="error-header-left"><h2 id="errorTitle">Error!</h2></div>
      ...
      <form id="uploadForm" action="#" method="post" enctype="multipart/form-data">
        <label for="f_file_name">
          <span style="color: red;">*</span> Choose a File (Only .txt)
        </label>
...
async function uploadFileAjax() {
  ...
  const url = "<?= site_url('eg/qcgibson/doupload'); ?>";
  ...
  try {
    ...
    catch(e) { showErrorCard("Server response is not valid JSON."); return; }
...
uploadForm.addEventListener("submit", function(e){
  e.preventDefault();
  if(fileInput.files.length === 0){ showErrorCard("No file selected."); return; }
  const fileExt = fileInput.files[0].name.split(".").pop().toLowerCase();
  if(fileExt !== "txt"){ showErrorCard("File must be a .txt file."); return; }
  uploadFileAjax();
});
