...
class Report_model extends Model
{
	...
	function getListPo($sCriteria = '', $nLimit = 0, $nOffset = 0, $aOrderby = array())
	{
		...
		// 		ttp.d_target_date, ttp.s_buyer, ttp.s_buyer_name, ttp.s_location, ttp.s_notes1, ttp.s_notes2,
		...
		// FROM 	tv_production_stock_qty_eg_3 AS ttp 

		// $sCriteria 
		// GROUP BY ttp.s_po_no, ttp.s_po, 
		// 		EXTRACT(YEAR FROM ttp.d_production_date) || '-' || EXTRACT(MONTH FROM ttp.d_production_date), 
		// 		ttp.d_plan_date, 
		// 		ttp.d_delivery_date, ttp.d_target_date, ttp.s_buyer, ttp.s_buyer_name, ttp.s_location, ttp.s_notes1, ttp.s_notes2
	...
	function getListColor($sCriteria = '', $nLimit = 0, $nOffset = 0, $aOrderby = array())
	{
		...
		$oQuery = $this->db->query("
			...
			GROUP BY ttp.s_color, ttp.s_color_name,
					EXTRACT(YEAR FROM ttp.d_production_date) || '-' || EXTRACT(MONTH FROM ttp.d_production_date), 
					ttp.d_plan_date, ttp.d_delivery_date, 
					ttp.s_po_no, ttp.s_po, ttp.d_target_date, ttp.s_buyer, ttp.s_buyer_name, ttp.s_model, ttp.s_model_name, 					ttp.s_smodel, ttp.s_location, ttp.s_notes1
			$sOrderBy $sLimitRows ");
	...
	function getRekapOPE($year, $case)
	{
		...
		switch ($case) {
			case 'order':
				$oQuery = $this->db->query("SELECT 
					COUNT(s_serial_no) AS order
					FROM tt_production ttp
					WHERE EXTRACT(YEAR FROM d_production_date) = $year
					AND ttp.s_division = 'EG'
					GROUP BY EXTRACT(YEAR FROM ttp.d_production_date)");
				break;

			case 'packing':
				$oQuery = $this->db->query("SELECT 
					COUNT(d_process_10) AS packing
					FROM tt_production ttp
					WHERE EXTRACT(YEAR FROM d_production_date) = $year
					AND ttp.s_division = 'EG'
					AND d_process_10 IS NOT NULL
					GROUP BY EXTRACT(YEAR FROM ttp.d_production_date)");
				break;

			case 'export':
				$oQuery = $this->db->query("SELECT 
					COUNT(d_process_14) AS export
					FROM tt_production ttp
					WHERE EXTRACT(YEAR FROM d_production_date) = $year
					AND ttp.s_division = 'EG'
					AND d_process_14 IS NOT NULL
					GROUP BY EXTRACT(YEAR FROM ttp.d_production_date)");
				break;
