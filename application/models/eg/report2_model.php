...
class Report2_model extends Model {
		...
		$oQuery=$this->db->query("
			SELECT 	ttp.s_po_no, ttp.s_po, 
					...
					(SUM(ttp.n_process_1) + SUM(ttp.n_process_2) + SUM(ttp.n_process_3) + SUM(ttp.n_process_4) + SUM(ttp.n_process_5) + SUM(ttp.n_process_6) +
					 SUM(ttp.n_process_1s) + 
					 SUM(ttp.n_process_7) + SUM(ttp.n_process_8) + SUM(ttp.n_process_9) + SUM(ttp.n_process_10) + SUM(ttp.n_process_14) ) AS n_qty
			FROM 	tv_production_qty_eg AS ttp 
			$sCriteria 
		...
		$oQuery=$this->db->query("
			SELECT 	ttp.s_po_no, ttp.s_po, ttp.s_lot_no, 
					...
					(SUM(ttp.n_process_1) + SUM(ttp.n_process_2) + SUM(ttp.n_process_3) + SUM(ttp.n_process_4) + SUM(ttp.n_process_5) + SUM(ttp.n_process_6) +
					 SUM(ttp.n_process_1s) + 
					 SUM(ttp.n_process_7) + SUM(ttp.n_process_8) + SUM(ttp.n_process_9) + SUM(ttp.n_process_10) + SUM(ttp.n_process_14) ) AS n_qty
			FROM 	tv_production_qty_eg AS ttp $sCriteria 
		...
		$oQuery=$this->db->query("
			SELECT 	ttp.s_po_no, ttp.s_po, ttp.s_model, ttp.s_model_name, ttp.s_smodel, ttp.s_color_name,
					...
					(SUM(ttp.n_process_1) + SUM(ttp.n_process_2) + SUM(ttp.n_process_3) + SUM(ttp.n_process_4) + SUM(ttp.n_process_5) + SUM(ttp.n_process_6) +
					 ...
					 SUM(ttp.n_process_7) + SUM(ttp.n_process_8) + SUM(ttp.n_process_9) + SUM(ttp.n_process_10) + SUM(ttp.n_process_14) ) AS n_qty
			FROM 	tv_production_qty_eg AS ttp $sCriteria 
			GROUP BY ttp.s_model, ttp.s_model_name, ttp.s_smodel, ttp.s_color_name, ttp.s_po_no, ttp.s_po, ttp.s_buyer, ttp.s_buyer_name, 
					EXTRACT(YEAR FROM ttp.d_production_date) || '-' || EXTRACT(MONTH FROM ttp.d_production_date),
	...
	function getListSerialDate( $sCriteria='', $nLimit=0, $nOffset=0, $aOrderby=array() ){
        ...
        $oQuery = $this->db->query("
			SELECT 	ttp.d_production_date, ttp.d_plan_date, ttp.d_delivery_date, ttp.d_target_date, 
					ttp.s_serial_no, ttp.s_buyer_name, ttp.s_po_no, ttp.s_po, ttp.s_lot_no,
					ttp.s_model,
					ttp.s_model_name, 
					ttp.s_color_name,
					ttp.s_smodel,
					...
					ttp.d_process_10, 
					ttp.d_process_14
			FROM 	tv_production_date_eg ttp	LEFT JOIN tv_production_date_eg_2 ttp2	ON (ttp.s_serial_no_2 = ttp2.s_serial_no)
			$sCriteria 
			$sOrderBy 
			$sLimitRows ");
	...
	function getListSerialDate2( $sCriteria='', $nLimit=0, $nOffset=0, $aOrderby=array() ){
        ...
        $oQuery = $this->db->query("
			SELECT 	ttp.d_production_date, ttp.d_plan_date, ttp.d_delivery_date, ttp.d_target_date, 
					ttp.s_serial_no, ttp.s_buyer_name, ttp.s_po_no, ttp.s_po, ttp.s_lot_no, ttp.s_model, ttp.s_model_name,
					ttp.s_smodel, ttp.s_color_name, 
					...
					ttp.d_process_10, 
					ttp.d_process_14
			FROM 	tv_production_date_eg ttp
			$sCriteria 
			$sOrderBy 
			$sLimitRows ");
